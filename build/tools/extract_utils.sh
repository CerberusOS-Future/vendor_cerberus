...
function setup_vendor() {
    ...
    export CERBERUS_ROOT="$3"
    if [ ! -d "$CERBERUS_ROOT" ]; then
        echo "\$CERBERUS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$CERBERUS_ROOT/$OUTDIR" ]; then
        mkdir -p "$CERBERUS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$CERBERUS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$CERBERUS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$CERBERUS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CERBERUS_ROOT"/vendor/cerberus/build/tools/smali/baksmali.jar
        export SMALIJAR="$CERBERUS_ROOT"/vendor/cerberus/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$CERBERUS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$CERBERUS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$CERBERUS_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$CERBERUS_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$CERBERUS_ROOT"/vendor/cerberus/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$CERBERUS_ROOT"/"$OUTDIR"/radio
